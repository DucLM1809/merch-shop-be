...
function injectUser(accountId: string, email: string) {
  return {
    canActivate: (ctx: ExecutionContext) => {
      ctx.switchToHttp().getRequest().user = { userId: accountId, email };
      return true;
    },
  };
}
...
async function seedAccount(p: PrismaService, email: string, role: 'BUYER' | 'ADMIN' = 'BUYER') {
  return p.account.create({ data: { email, role } });
}
...
// AdminGuard injects AuthGuard directly (not via @UseGuards), so
// overrideGuard(AuthGuard) never reaches it. Re-create the real role
// check against Prisma instead, same pattern as admin.integration.spec.ts.
async function buildAdminCheckedApp(accountId: string) {
  const moduleRef = await Test.createTestingModule({ imports: [AppModule] })
    ...
    .useFactory({
      ...
      factory: (p: PrismaService) => ({
        canActivate: async (ctx: ExecutionContext) => {
          ...
          const account = await p.account.findUnique({ where: { id: accountId }, select: { role: true } });
...
describe('GET /api/orders/mine', () => {
  ...
  beforeAll(async () => {
    // Seed the module once to get a real Prisma handle, then create the
    // account before building the request-serving app — the guard override
    // needs the account's real id, not an arbitrary placeholder.
    const bootstrap = await Test.createTestingModule({ imports: [AppModule] }).compile();
    prisma = bootstrap.get(PrismaService);
    const account = await seedAccount(prisma, BUYER_A_EMAIL);
    accountId = account.id;
    await bootstrap.close();

    const built = await buildApp(injectUser(accountId, BUYER_A_EMAIL));
    app = built.app;
    prisma = built.prisma;
...
describe('GET /api/orders/:id', () => {
  ...
  beforeAll(async () => {
    const bootstrap = await Test.createTestingModule({ imports: [AppModule] }).compile();
    const bootstrapPrisma = bootstrap.get(PrismaService);
    const accountA = await seedAccount(bootstrapPrisma, BUYER_A_EMAIL);
    accountAId = accountA.id;
    const accountB = await seedAccount(bootstrapPrisma, BUYER_B_EMAIL);
    accountBId = accountB.id;
    await bootstrap.close();

    const built = await buildApp(injectUser(accountAId, BUYER_A_EMAIL));
    app = built.app;
    prisma = built.prisma;
    ordersService = built.orders;
  ...
  it('404s for a Buyer requesting another Account Order', async () => {
    const other = await buildApp(injectUser(accountBId, BUYER_B_EMAIL));
...
describe('GET /api/orders (admin)', () => {
  ...
  beforeAll(async () => {
    ...
    const accountB = await seedAccount(bootstrapPrisma, BUYER_B_EMAIL);
    accountBId = accountB.id;
    const admin = await seedAccount(bootstrapPrisma, ADMIN_EMAIL, 'ADMIN');
  ...
  it('403s for a non-admin Buyer', async () => {
    const buyerChecked = await buildAdminCheckedApp(accountAId);
  ...
  it('returns all Orders for an Admin, not just their own', async () => {
    const adminChecked = await buildAdminCheckedApp(adminAccountId);
